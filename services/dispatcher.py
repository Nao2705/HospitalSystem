...
class Dispatcher:
    ...
    def on_patient_arrival(self, patient: Patient, current_time: float) -> bool:
        ...
        print(f"Время {current_time:.2f}: Прибыл {patient}")

        # Пытаемся добавить пациента в буфер
        if self.waiting_room.add_patient(patient):
            ...
            print(f"{patient.name} получил отказ - буфер полон")
            # Регистрируем отказ в статистике
    ...
    def _try_assign_patient_from_buffer(self, current_time: float) -> None:
        ...
        Выбирает пациента по Д2Б4 (высший приоритет) и врача по Д2П2 (кольцевой).
        """
    ...
    def _find_free_doctor(self) -> Optional[Doctor]:
        """Находит свободного врача по кольцевому алгоритму (Д2П2)"""
        if not self.doctors:
            return None
    ...
    def get_system_state(self) -> dict:
        ...
        free_doctors = [d for d in self.doctors if not d.get_is_busy()]
        busy_doctors = [d for d in self.doctors if d.get_is_busy()]
        ...
        return {
            ...
            'busy_doctors_list': [f"{d.name} с {d.get_current_patient().name}" for d in busy_doctors if
                                  d.get_current_patient()]
        }
